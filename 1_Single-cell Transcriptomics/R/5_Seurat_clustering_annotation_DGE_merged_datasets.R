...
#BM_integrated <- readRDS("./RDS_files/Merged_datasets/BM_merged_integrated.rds")
#saveRDS(BM_integrated, "./RDS_files/Merged_datasets/BM_merged_integrated.rds")

saveRDS(BM_query, "./BM_merged_query.rds")

BM_query <- readRDS("./RDS_files/Merged_datasets/BM_merged_query.rds")

#ref <- readRDS("./Reference_datasets/HCA/ref.rds")
#ref2 <- readRDS("./Reference_datasets/Seurat/Seurat_Multimodal_Reference.rds")
...
Idents(BM_query) <- 'predicted.celltype.l2'
levels(BM_query)
levels(BM_query) <- c("Stromal", "HSC", "LMPP", "EMP", "Prog Mk", "Platelet",
                      "Early Eryth", "Late Eryth", "BaEoMa",  "GMP", "pre-mDC", "cDC1","cDC2", "ASDC", "pre-pDC","pDC", "CD14 Mono", "CD16 Mono", 
                      "Macrophage", "CLP", "pro B", "pre B", "transitional B", "Naive B", "Memory B", "Plasma", "T Proliferating","CD4 Naive", "CD8 Naive",
                      "CD4 Effector", "CD4 Memory", "CD8 Effector_1", "CD8 Effector_2", "CD8 Effector_3" ,"CD8 Memory", "NK", "NK CD56+", "NK Proliferating", "ILC", "MAIT")


BM_query$predicted.celltype.l2.ordered <- factor(BM_query@active.ident, 
                                                 levels=c("Stromal", "HSC", "LMPP", "EMP", "Prog Mk", "Platelet",
                                                         "Early Eryth", "Late Eryth", "BaEoMa",  "GMP", "pre-mDC", "cDC1","cDC2", "ASDC", "pre-pDC","pDC", "CD14 Mono", "CD16 Mono", 
                                                         "Macrophage", "CLP", "pro B", "pre B", "transitional B", "Naive B", "Memory B", "Plasma", "T Proliferating","CD4 Naive", "CD8 Naive",
                                                         "CD4 Effector", "CD4 Memory", "CD8 Effector_1", "CD8 Effector_2", "CD8 Effector_3" ,"CD8 Memory", "NK", "NK CD56+", "NK Proliferating", "ILC", "MAIT"))
...
VlnPlot(BM_query, features = proteins1, assay = "dsb", slot = "data", ncol = 2, pt.size = 0.25, sort = TRUE, combine = TRUE) + NoLegend()
VlnPlot(BM_query, features = proteins2,  assay = "dsb",slot = "data", ncol = 2, pt.size = 0.25, sort = TRUE, combine = TRUE) + NoLegend()

RidgePlot(BM_query, features = proteins1,  assay = "dsb", slot = "data", ncol = 2, sort = TRUE, combine = TRUE) 
RidgePlot(BM_query, features = proteins2,  assay = "dsb", slot = "data", ncol = 2, sort = TRUE, combine = TRUE) 
...
BM_query <- FindClusters(BM_query, graph.name = "predicted.ADT.wsnn", 
                         ...
                         resolution = 1.5, 
                         verbose = FALSE, 
                         random.seed = 1990, cluster.name = "WNN_clusters")
...
dittoDimPlot(BM_query, var = "predicted.celltype.l2",
  ...
  theme(legend.title = element_blank()) + DarkTheme()
...
BM_query <- RunUMAP(BM_query, reduction = 'pca', dims = 1:30, assay = 'RNA', 
                     reduction.name = 'rna.umap', reduction.key = 'rnaUMAP_')
BM_query <- RunUMAP(BM_query, reduction = 'apca', dims = 1:7, assay = 'predicted_ADT', 
...
p3 <- DimPlot(BM_query, reduction = 'rna.umap', group.by = 'predicted.celltype.l2', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend()
p4 <- DimPlot(BM_query, reduction = 'predicted.adt.umap', group.by = 'predicted.celltype.l2', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend()
p3 + p4
...
# 18- Differential abundance testing with Milo
...
# 18.1- Milo set-up and make milo object
DefaultAssay(BM_query) <- "RNA" 
Idents(BM_query) <- 'predicted.celltype.l2'
levels(BM_query)

BM_sce <- as.SingleCellExperiment(BM_query, assay = "RNA")
saveRDS(BM_sce, "BM_merged_sce.rds")

milo <- Milo(BM_sce)
milo
saveRDS(milo, "BM_merged_milo.rds")

# 18.2- Add KNN graph if there is no KNN graph in the SCE object 

# Milo looks for neighbourhoods in a KNN graph to perform DA analysis. 
# This need to be stored in the graph slot of the Milo object. 
# We can recompute the KNN graph using the dedicated function in miloR. USE THIS!
milo <- buildGraph(milo, k=20, d = 50, transposed = FALSE) #k = 20, similar to the FindNeighbors function from Seurat

# 18.3- Defining representative neighbourhoods on the KNN graph
...
milo <- makeNhoods(milo, prop = 0.05, k = 20, d=50, refined = TRUE, reduced_dims = "PCA")
...
# 18.4- Counting cells in neighbourhoods
...
milo <- countCells(milo, meta.data = as.data.frame(colData(milo)), sample="sample_id")
...
# 18.5- Defining experimental design
...
milo_design <- data.frame(colData(milo))[,c("sample_id","experiment_date")]

## Convert batch info from integer to factor
milo_design$experiment_date <- as.factor(milo_design$experiment_date) 
milo_design <- distinct(milo_design)
rownames(milo_design) <- milo_design$sample_id
milo_design

# 18.6- Computing neighbourhood connectivity
...
milo <- calcNhoodDistance(milo, d=50, reduced.dim = "PCA") #it takes a long time to run!
saveRDS(milo, "BM_merged_milo.rds")
...
table(milo_design$sample_id)
table(milo_design$experiment_date)
milo_design <- milo_design[colnames(nhoodCounts(milo)), , drop=FALSE]

contrast.1 <- c("experiment_date12_18 - experiment_date10_22") # the syntax is <VariableName><ConditionLevel> - <VariableName><ControlLevel>

da_results1 <- testNhoods(milo, design = ~ 0 + experiment_date, design.df = milo_design, model.contrasts = contrast.1)
head(da_results1)
da_results1 %>%
  arrange(SpatialFDR) %>%
  head() 
...
# 18.8- Inspecting DA testing results
...
ggplot(da_results1, aes(logFC, -log10(SpatialFDR))) + 
  geom_point() +
...
umap_pl <- plotReducedDim(milo, dimred = "PROJ.UMAP", colour_by="experiment_date", text_by = "predicted.celltype.l2", 
...
nh_graph_pl <- plotNhoodGraphDA(milo, da_results1, layout="PROJ.UMAP",alpha=0.1) 

umap_pl + nh_graph_pl +
  plot_layout(guides="collect")

# We might also be interested in visualizing whether DA is particularly evident in certain cell types. 
...
da_results1 <- annotateNhoods(milo, da_results1, coldata_col = "predicted.celltype.l2")
head(da_results1)
plotDAbeeswarm(da_results1, group.by = "predicted.celltype.l2")
...
ggplot(da_results1, aes(predicted.celltype.l2_fraction)) + geom_histogram(bins=50)

da_results1$predicted.celltype.l2 <- ifelse(da_results1$predicted.celltype.l2_fraction < 0.7, "Mixed", da_results1$predicted.celltype.l2)

# Now we can visualize the distribution of DA Fold Changes in different cell types after filtering
plotDAbeeswarm(da_results1, group.by = "predicted.celltype.l2")

# 18.9- Finding markers of DA populations
...
da_nhood_markers1 <- findNhoodGroupMarkers(milo, da_results1)
head(da_nhood_markers1)

plotNhoodGroups(milo, da_results1, layout="PROJ.UMAP") 

ggplot(da_nhood_markers1, aes(logFC_0,-log10(adj.P.Val_0 ))) + 
  geom_point() +
  geom_hline(yintercept = 2) #try with enhanced volcano

rownames(da_nhood_markers1) <- da_nhood_markers1$GeneID
markers <- rownames(da_nhood_markers1)[da_nhood_markers1$adj.P.Val_0 < 0.001 & da_nhood_markers1$logFC_0 > 0.25]
plotNhoodExpressionGroups(milo, da_results1, features=markers,
                          subset.nhoods = da_results1$NhoodGroup %in% c('0','1'), scale=TRUE,
                          grid.space = "fixed")
...
# 20- Automated cell annotation using SingleR
...
saveRDS(hemato, "NovershternHematopoieticData.rds")
...
colnames(pred)
saveRDS(pred, "./RDS_files/Merged_datasets/pred.rds")
pred <- readRDS("./RDS_files/Merged_datasets/pred.rds")

pred2 <- SingleR(test = BM_sce, ref = ref.data, 
                labels = ref.data$label.fine, assay.type.test=1)
colnames(pred2)
saveRDS(pred2, "./RDS_files/Merged_datasets/pred2.rds")

pred3 <- SingleR(test = BM_sce, ref = blueprint, 
                labels = blueprint$label.fine, assay.type.test=1)
colnames(pred3)
saveRDS(pred3, "./RDS_files/Merged_datasets/pred3.rds")
...
BM_query[["SingleR.labels2"]] <- pred2$labels
BM_query[["SingleR.labels3"]] <- pred3$labels

# Visualise Single R labels in the UMAP from one of the BM reference
dittoDimPlot(BM_query, var = "SingleR.labels3",
             reduction.use = "proj.umap", size = 0.75,
             do.label = TRUE, labels.size = 4, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
  ggtitle("SingleR cell types on UMAP")

DimPlot(BM_query, group.by = 'SingleR.labels2', reduction = "proj.umap", label = TRUE) + NoLegend()
...
VlnPlot(BM_query, features = proteins3, slot = "data", ncol = 2, pt.size = 0.1, sort = TRUE, combine = TRUE) + NoLegend()
VlnPlot(BM_query, features = proteins4, slot = "data", ncol = 2, pt.size = 0.1, sort = TRUE, combine = TRUE) + NoLegend()
VlnPlot(BM_query, features = proteins5, slot = "data", ncol = 2, pt.size = 0.1, sort = TRUE, combine = TRUE) + NoLegend()
...
# 20.3- Annotation diagnostics

# n addition to the labels, SingleR() returns a number of helpful diagnostics about the annotation process that can be used to determine whether the assignments are appropriate. 
# Unambiguous assignments corroborated by expression of canonical markers add confidence to the results; conversely, low-confidence assignments can be pruned out to avoid adding noise to downstream analyses.

# 20.3.1- Based on the scores within cells
...
# To check whether this is indeed the case, we use the plotScoreHeatmap() function to visualize the score matrix.
plotScoreHeatmap(pred)

# We can also display other metadata information for each cell by setting clusters= or annotation_col=. 
# This is occasionally useful for examining potential batch effects, differences in cell type composition between conditions, relationship to clusters from an unsupervised analysis and so on. 
# £ach cell type has contributions from multiple donors, which is reassuring as it indicates that our assignments are not (purely) driven by donor effects.
# The scores matrix has several caveats associated with its interpretation. Only the pre-tuned scores are stored in this matrix, as scores after fine-tuning are not comparable across all labels. 
# This means that the label with the highest score for a cell may not be the cell’s final label if fine-tuning is applied. 
# Moreover, the magnitude of differences in the scores has no clear interpretation; indeed, plotScoreHeatmap() dispenses with any faithful representation of the scores and instead adjusts the values to highlight any differences between labels within each cell.
plotScoreHeatmap(pred, annotation_col=as.data.frame(colData(BM_sce)[,"sample_id",drop=FALSE]))

# 20.3.2- Based on the deltas across cells

# We identify poor-quality or ambiguous assignments based on the per-cell “delta”, i.e., the difference between the score for the assigned label and the median across all labels for each cell. 
# Our assumption is that most of the labels in the reference are not relevant to any given cell. 
# Thus, the median across all labels can be used as a measure of the baseline correlation, while the gap from the assigned label to this baseline can be used as a measure of the assignment confidence.
# Low deltas indicate that the assignment is uncertain, possibly because the cell’s true label does not exist in the reference. An obvious next step is to apply a threshold on the delta to filter out these low-confidence assignments. 
# We use the delta rather than the assignment score as the latter is more sensitive to technical effects. 
# For example, changes in library size affect the technical noise and can increase/decrease all scores for a given cell, while the delta is somewhat more robust as it focuses on the differences between scores within each cell.
# SingleR() will set a threshold on the delta for each label using an outlier-based strategy. Specifically, we identify cells with deltas that are small outliers relative to the deltas of other cells with the same label. 
# This assumes that, for any given label, most cells assigned to that label are correct. 
# We focus on outliers to avoid difficulties with setting a fixed threshold, especially given that the magnitudes of the deltas are about as uninterpretable as the scores themselves. 
# Pruned labels are reported in the pruned.labels field where low-quality assignments are replaced with NA.
# This entire process can be visualized using the plotScoreDistribution() function, which displays the per-label distribution of the deltas across cells.
# We can use this plot to check that outlier detection in pruneScores() behaved sensibly. Labels with especially low deltas may warrant some additional caution in their interpretation.
...
# The example below discards cells with deltas below an arbitrary threshold of 0.2, where higher thresholds correspond to greater assignment certainty.
to.remove2 <- pruneScores(pred, min.diff.med=0.1)
table(Label=pred$labels, Removed=to.remove)
plotDeltaDistribution(pred, size = 0.5)

# 20.3.3- Based on marker gene expression
...
eryth.markers <- unique(unlist(all.markers$`Erythroid_CD34+ CD71+ GlyA-`))
BM_sce$labels <- pred$labels
#BM_sce$labels <- as.factor(BM_sce$labels)
saveRDS(BM_sce, "./RDS_files/Merged_datasets/BM_merged_sce.rds")

library(scater)
plotHeatmap(BM_sce, order_columns_by="labels", features=eryth.markers)
...
library(scran)
empirical.markers <- findMarkers(BM_sce, BM_sce$labels, direction="up")
m <- match(eryth.markers, rownames(empirical.markers$`Erythroid_CD34+ CD71+ GlyA-`))
m <- eryth.markers[rank(m) <= 20]

library(scater)

#cell_colors = c(dittoColors()[seq_len(38)])

plotHeatmap(BM_sce, order_columns_by="labels", features=m)

dittoHeatmap(BM_sce, m, assay = "logcounts", annot.by = c("labels"), 
             heatmap.colors = colorRampPalette(c("blue", "white", "red"))(10), 
             heatmap.colors.max.scaled = inferno(10), cluster_rows = FALSE, scaled.to.max = FALSE, cluster_cols = FALSE,
             fontsize_row = 12)

dittoHeatmap(BM_sce, m, annot.by = c("labels"))

DotPlot(BM_query, features = m, group.by = "SingleR.labels", assay = "RNA", cols = c("lightgrey", "blue")) + RotatedAxis()

# It is straightforward to repeat this process for all labels by wrapping this code in a loop, as shown below in Figure 4.6.
# Note that plotHeatmap() is not the only function that can be used for this visualization; 
# we could also use plotDots() to create a Seurat-style dot plot, or we could use other heatmap plotting functions such as dittoHeatmap() from dittoSeq.
collected <- list()
...
# 21- Automated cell annotation using CIPR
...
set.seed(1234)
DefaultAssay(BM_query) <- "SCT"
BM_query <- FindNeighbors(BM_query, reduction = "harmony")
BM_query <- FindClusters(BM_query, resolution = 1, cluster.name = "harmony_clusters")

DefaultAssay(BM_query) <- "RNA" 
Idents(BM_query) <- 'WNN_clusters' #these will do for the 76 WNN (ADT+RNA) clusters 
levels(BM_query)

DefaultAssay(BM_query) <- "RNA" 
Idents(BM_query) <- 'harmony_clusters' #these will do for the 41 RNA clusters 
levels(BM_query)

# Look at cluster IDs of the first 5 cells
head(Idents(BM_subset), 5)

# 21.1- Calculate average gene expression per cluster
# This is the step where we generate the input for CIPR's all-genes correlation methods
avgexp <- AverageExpression(BM_query)
avgexp <- as.data.frame(avgexp$RNA)
avgexp$gene <- rownames(avgexp)
write.csv(avgexp, "avgexp_BM_query_merged_harmony_clusters.csv")

avgexp <- read.csv("./Plots_merged_datasets/12_CIPR/avgexp_BM_query_merged.csv")

# 21.1- Calculate differentially expressed genes per cluster
allmarkers <- FindAllMarkers(object = BM_query, only.pos = FALSE, min.pct = 0.25, logfc.threshold = 0.25)
clipr::write_clip(allmarkers) # Copies the table to the clipboard. Go to excel and press cmd+V (mac) or ctrl+V (windows) to paste

#cluster75 <- FindMarkers(BM_query, ident.1 = "75", ident.2 = NULL, only.pos = FALSE, min.pct = 0.25, logfc.threshold = 0.25)
#clipr::write_clip(cluster74) # Copies the table to the clipboard. Go to excel and press cmd+V (mac) or ctrl+V (windows) to paste

allmarkers <- read.csv("./Plots_merged_datasets/12_CIPR/BM_markers_seurat_clusters_RNA.csv")

# 21.2- CIPR analysis

# 21.2.1- Standard logFC comparison method
...
write.csv(CIPR_top_results, "CIPR_top_results_hema_harmony_clusters.csv")
write.csv(CIPR_all_results, "CIPR_all_results_hema_harmony_clusters.csv")

# 21.2.2- Standard all-genes correlation method
...
write.csv(CIPR_top_results, "CIPR_top_results_corr_hema_harmony_clusters.csv")
write.csv(CIPR_all_results, "CIPR_all_results_corr_hema_harmony_clusters.csv")


# 21.3- Add CIPR cluster labels

# 21.3.1- Annotate harmony clusters
Idents(BM_query) <- "harmony_clusters"
levels(BM_query)
options(ggrepel.max.overlaps = Inf)

dittoDimPlot(BM_query, var = "harmony_clusters",
             reduction.use = "proj.umap", size = 1,
             do.label = TRUE, labels.size = 2, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
  ggtitle("Clusters on UMAP")

# based on hema reference and logFC method
CIPR_harmony_clusters <- c("CM CD8+ T cell", "CD34-CD71loGlyA+ Erythroid cell", "CM CD8+ T cell", "CD56+CD16+CD3- Mature NK cell",
                           "Monocyte", "Naive B cell", "CM CD8+ T cell", "Monocyte", "CD34-CD71+GlyA+ Erythroid cell",
                           "Naive CD4+ T cell", "CM CD8+ T cell", "CD34-CD71loGlyA+ Erythroid cell", "CD34-CD71+GlyA+ Erythroid cell",
                           "Naive CD4+ T cell", "EM CD8+ T cell", "Naive CD8+ T cell", "CD34-CD71loGlyA+ Erythroid cell", "Naive B cell", "EM CD8+ T cell", "Naive B cell", "CD34-CD71+GlyA+ Erythroid cell",
                           "HSC", "Naive CD8+ T cell", "EM CD8+ T cell", "Naive B cell", "CMP", "Early B cell", "CD34+CD71+GlyA- Erythroid cell" , "CD34-CD71loGlyA+ Erythroid cell", 
                           "Plasmacytoid DC", "Monocyte", "Myeloid DC", "CD34-CD71-GlyA+ Erythroid cell", "Monocyte", "CD34+CD71+GlyA- Erythroid cell", "CD34-CD71loGlyA+ Erythroid cell", "MEP", 
                          "Mature B cells class able to switch", "Basophil", "Early B cell", "CD34-CD71loGlyA+ Erythroid cell")
                
# based on hema reference and spearman correlation method
CIPR_harmony_clusters_v2 <- c("Naive CD8+ T cell", "CD34-CD71loGlyA+ Erythroid cell", "Naive CD8+ T cell", "CD56+CD16+CD3- Mature NK cell",
                           "Monocyte", "Naive B cell", "Naive CD8+ T cell", "Monocyte", "CD34-CD71+GlyA+ Erythroid cell",
                           "Naive CD4+ T cell", "Naive CD4+ T cell", "CD34-CD71loGlyA+ Erythroid cell", "CD34-CD71+GlyA+ Erythroid cell",
                           "Naive CD8+ T cell", "EM CD8+ T cell", "Naive CD8+ T cell", "CD34-CD71loGlyA+ Erythroid cell", "Mature B cells class switched", "CD56+CD16+CD3- Mature NK cell", "Naive B cell", "CD34-CD71+GlyA+ Erythroid cell",
                           "HSC", "Naive CD4+ T cell", "EM CD8+ T cell", "Naive B cell", "CD133+ CD34dim HSC", "Mature B cells class switched", "CD34+CD71+GlyA- Erythroid cell" , "CD34-CD71loGlyA+ Erythroid cell", 
                           "Plasmacytoid DC", "Monocyte", "Myeloid DC", "CD34-CD71-GlyA+ Erythroid cell", "Monocyte", "CD34+CD71+GlyA- Erythroid cell", "CD34-CD71loGlyA+ Erythroid cell", "MEP", 
                           "Mature B cells class able to switch", "Basophil", "Early B cell", "CD34-CD71loGlyA+ Erythroid cell")
                           

names(CIPR_harmony_clusters) <- levels(BM_query)
BM_query <- RenameIdents(BM_query, CIPR_harmony_clusters)

# Adding object metadata with
BM_query$CIPR_harmony_clusters <- factor(BM_query@active.ident)

dittoDimPlot(BM_query, var = "CIPR_harmony_clusters",
             reduction.use = "proj.umap", size = 1,
             do.label = TRUE, labels.size = 3, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
  ggtitle("Clusters on UMAP")

DimPlot(BM_query, group.by = 'CIPR_harmony_clusters', reduction = "proj.umap", label = TRUE) + NoLegend()

# Frequency and total number of dsb clusters
dittoBarPlot(BM_query, var = "CIPR_harmony_clusters", group.by = "sample_id", retain.factor.levels = TRUE)
dittoBarPlot(BM_query, var = "CIPR_harmony_clusters", group.by = "sample_id", retain.factor.levels = TRUE, scale = "count")

# Plot normalised protein expression per cluster
DefaultAssay(BM_query) <- "predicted_ADT"
Idents(BM_query) <- "CIPR_harmony_clusters"
levels(BM_query)
...
DefaultAssay(BM_query) <- "RNA" 
CIPR_harmony_clusters.markers <- FindAllMarkers(BM_query, only.pos = TRUE, logfc.threshold = 0.25, min.pct = 0.25) 
clipr::write_clip(CIPR_harmony_clusters.markers) # Copies the table to the clipboard. Go to excel and press cmd+V (mac) or ctrl+V (windows) to paste

# DoHeatmap() generates an expression heatmap for given cells and features. 
# In this case, we are plotting the top 20 markers (or all markers if less than 20) for each cluster.
CIPR_harmony_clusters.markers %>%
  group_by(cluster) %>%
  slice_max(n = 10, order_by = avg_log2FC) -> top10
...
dittoHeatmap(BM_query, top10$gene, 
             cluster_rows = FALSE, 
             annot.by = "CIPR_harmony_clusters", 
             fontsize_row = 8)
...
# 21.3.2- Annotate WNN clusters
Idents(BM_query) <- "WNN_clusters"
levels(BM_query) #pay attention to the order of the clusters when annotating them!

dittoDimPlot(BM_query, var = "WNN_clusters",
             reduction.use = "proj.umap", size = 1,
             do.label = TRUE, labels.size = 1, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
  ggtitle("Clusters on UMAP")

BM_query$CIPR_WNN_clusters <- NULL

# based on hema reference and logFC method
CIPR_WNN_clusters <- c("CM CD8+ T cell", "Naive B cell", "EM CD4+ T cell", "Naive CD8+ T cell", "Monocyte", "CD34-CD71+GlyA+ Erythroid cell", "Naive CD4+ T cell",
                       "CD34-CD71loGlyA+ Erythroid cell", "CM CD4+ T cell", 
                       "Naive CD4+ T cell", "CD34+CD71+GlyA- Erythroid cell", "CD56+CD16+CD3- Mature NK cell", "EM CD4+ T cell",
                       "CM CD8+ T cell", "EM CD4+ T cell", "HSC", "Naive B cell", "CD34-CD71loGlyA+ Erythroid cell", "Pre B cell", 
                       "CD34-CD71-GlyA+ Erythroid cell", "Neutrophil", "CD34-CD71loGlyA+ Erythroid cell", "CFU-Monocyte", "CM CD8+ T cell", "Neutrophil", "CD34-CD71loGlyA+ Erythroid cell", "EM CD4+ T cell", "EM CD8+ T cell", "CD34-CD71loGlyA+ Erythroid cell",
                       "EM CD4+ T cell", "Myeloid DC", "CD34-CD71+GlyA+ Erythroid cell", "Early B cell", "CD34-CD71+GlyA+ Erythroid cell", "CD34-CD71loGlyA+ Erythroid cell", "MEP", "Pro B cell", "CD34-CD71+GlyA+ Erythroid cell", "Plasmacytoid DC", "Myeloid DC", "CD34-CD71+GlyA- Erythroid cell",
                       "Early B cell", "CMP", "EM CD8+ T cell", "Megakaryocyte", "Neutrophilic Metamyelocyte", "Pro B cell", "Naive CD4+ T cell", "EM CD8+ T cell", "Megakaryocyte", "CD34-CD71+GlyA- Erythroid cell", "Monocyte", 
                       "CD34-CD71loGlyA+ Erythroid cell", "Megakaryocyte", "Myeloid DC", "Plasmacytoid DC", "EM CD4+ T cell", "CD56-CD16-CD3- Mature NK cell", "CD34-CD71+GlyA+ Erythroid cell", "CM CD8+ T cell", "EM CD8+ T cell", "CD34-CD71+GlyA+ Erythroid cell", 
                       "CD34-CD71loGlyA+ Erythroid cell", "Megakaryocyte", "CD34+CD71+GlyA- Erythroid cell", "CD34-CD71-GlyA+ Erythroid cell", "Neutrophil", "CD34-CD71-GlyA+ Erythroid cell", "Naive B cell", "EM CD8+ T cell", "Monocyte", "EM CD8+ T cell",
                       "Naive CD4+ T cell", "Naive CD4+ T cell", "EM CD8+ T cell", "EM CD8+ T cell")

names(CIPR_WNN_clusters) <- levels(BM_query)
BM_query <- RenameIdents(BM_query, CIPR_WNN_clusters)

# Adding object metadata with
BM_query$CIPR_WNN_clusters <- factor(BM_query@active.ident)

Idents(BM_query) <- "CIPR_WNN_clusters"
levels(BM_query)

dittoDimPlot(BM_query, var = "CIPR_WNN_clusters",
             reduction.use = "proj.umap", size = 1,
             do.label = TRUE, labels.size = 3, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
  ggtitle("Clusters on UMAP")

DimPlot(BM_query, group.by = 'CIPR_WNN_clusters', reduction = "proj.umap", label = TRUE) + NoLegend()

# Frequency and total number of dsb clusters
dittoBarPlot(BM_query, var = "CIPR_WNN_clusters", group.by = "sample_id", retain.factor.levels = TRUE)
dittoBarPlot(BM_query, var = "CIPR_WNN_clusters", group.by = "sample_id", retain.factor.levels = TRUE, scale = "count")
...
multi_dittoPlot(BM_erythro, feats, group.by = "SingleR.labels_ordered",
                vlnplot.lineweight = 0.3, jitter.size = 0.3)
FeaturePlot(BM_erythro, feats, cols = viridis(100, direction = -1)) # cols changes the colors of the plot. I really like viridis for those heatmaps

DefaultAssay(BM_query) <- "RNA" 
Idents(BM_query) <- "CIPR_WNN_clusters"
levels(BM_query)

BM_erythro <- subset(BM_query, idents = c("MEP","CD34+CD71+GlyA- Erythroid cell", "CD34-CD71+GlyA- Erythroid cell", "CD34-CD71+GlyA+ Erythroid cell",
                                          "CD34-CD71loGlyA+ Erythroid cell", "CD34-CD71-GlyA+ Erythroid cell"))

Idents(BM_erythro) <- "CIPR_WNN_clusters"
levels(BM_erythro)
my_levels <- c("MEP","CD34+CD71+GlyA- Erythroid cell", "CD34-CD71+GlyA- Erythroid cell", "CD34-CD71+GlyA+ Erythroid cell",
                                          "CD34-CD71loGlyA+ Erythroid cell", "CD34-CD71-GlyA+ Erythroid cell")
# Relevel object@ident
BM_erythro@active.ident <- factor(x = BM_erythro@active.ident, levels = my_levels)
BM_erythro$SCIPR_WNN_ordered <- factor(BM_erythro@active.ident)
...
# 22- Trajectory Inference Analysis - StemID
# Reference: https://github.com/dgrun/StemID/blob/master/Reference_manual_RaceID2_StemID.pdf
require(tsne)
require(pheatmap)
require(MASS)
require(cluster)
...
# The S4 class object is specified in the R script RaceID2_StemID_class.R, which has to be loaded into the workspace.
source("./Plots_merged_datasets/14_StemID/StemID-master/RaceID2_StemID_class.R")

# 22.1- Input data
# Input data have to be provided as a data.frame object with expression values, i. e. raw read or transcript counts, with genes as rows and cells as columns. 
# Row names should correspond to gene identifiers and column names to identifiers of single cell samples.
DefaultAssay(BM_query) <- "RNA" 
data <- as(as.matrix(BM_query@assays$RNA@counts), 'sparseMatrix') 
data <- as.data.frame(data)

# Given a data.frame of transcript counts, an SCseq object can be created:
sc <- SCseq(data)
saveRDS(sc, "BM_query_merged_sc.rds")

# 22.2- Preprocessing of input data 
# The function call displays the default values of all arguments.
# After discarding cells with less than mintotal transcripts, the expression data are normalized. 
# Normalization is performed by dividing transcript counts in each cell by the total number of transcripts in this cell followed by multiplication with the median of the total number of transcripts across cells (median normalization). 
# If downsample is set to TRUE, then transcript counts are downsampled to mintotal transcripts per cell, instead of the normalization. 
# Downsampled versions of the transcript count data are averaged across dsn samples. If dsn equals one, sampling noise should be comparable across cells, while for high numbers of dsn the data will become similar to the median normalization.
# To ensure reproducibility of downsampling the random number generator is initialized with a fixed seed rseed that can be changed. By default downsample is set to TRUE.
...
# If expression data are derived based on unique molecular identifiers 2–4 (UMIs) it is recommended to filter out genes with saturated UMI counts, since differences in saturation between cells will affect the transcriptome correlation and potentially introduce spurious clusters. 
# This is done by discarding genes with at least maxexpr transcripts in at least a single cell after normalization or downsampling. 
# To disable this filtering step, maxexpr should be set to Inf. By default maxexpr is set to Inf. After gene filtering, the transcript count data overwrite the slot sc@fdata.
sc <- filterdata(sc, mintotal=3000, minexpr=5, minnumber=1, maxexpr=500, downsample=TRUE, dsn=1, rseed=17000)

# 22.3- k-medoids clustering
# The preprocessed data are clustered by k-medoids clustering.
sc <- clustexp(sc, clustnr=40, bootnr=50, metric="euclidean", do.gap=FALSE,
...
# 23- Trajectory Inference Analysis - PAGA
# Convert the BM_query_merged seurat object to anndata object in the COSMX scripts using renv and reticulate, so you dont need to install renv here.

library(sceasy)

sceasy::convertFormat(BM_query, from="seurat", to="anndata",
                      outFile='BM_merged_query.h5ad')

################################################################################################################################################################################

# 24- Subset Seurat object to cluster major cell types and re-add the info to the original seurat object
...
BM_eryth <- subset(BM_query, idents = c("Hematopoietic Stem Cell", "HSPC", "Erythroid", "Platelets"))
...
BM_lympho <- subset(BM_query, idents = c("CD4 T cell", "CD8 T cell", "Invariant T cell", "Natural Killer", "Innate Lymphocyte", "Proliferating T cell"))
BM_stem <- subset(BM_query, idents = c("Hematopoietic Stem Cell", "HSPC"))

saveRDS(BM_eryth, "./RDS_files/Merged_datasets/BM_merged_eryth.rds")
saveRDS(BM_myelo, "./RDS_files/Merged_datasets/BM_merged_myelo.rds")
saveRDS(BM_bcell, "./RDS_files/Merged_datasets/BM_merged_bcell.rds")
saveRDS(BM_lympho, "./RDS_files/Merged_datasets/BM_merged_lympho.rds")
saveRDS(BM_stem, "./RDS_files/Merged_datasets/BM_merged_stem.rds")

sceasy::convertFormat(BM_eryth, from="seurat", to="anndata",
                      outFile='BM_merged_eryth.h5ad')
sceasy::convertFormat(BM_myelo, from="seurat", to="anndata",
                      outFile='BM_merged_myelo.h5ad')
sceasy::convertFormat(BM_bcell, from="seurat", to="anndata",
                      outFile='BM_merged_bcell.h5ad')
sceasy::convertFormat(BM_lympho, from="seurat", to="anndata",
                      outFile='BM_merged_lympho.h5ad')
sceasy::convertFormat(BM_stem, from="seurat", to="anndata",
                      outFile='BM_merged_stem.h5ad')
